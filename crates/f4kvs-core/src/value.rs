...
impl Value {
    ...
    pub fn from_json_bytes(json: &[u8]) -> crate::Result<Self> {
        ...
        let serde_value =
            simd_json::from_slice::<SerdeJsonValue>(&mut bytes).map_err(crate::F4KvsError::from)?;
...
mod tests {
    ...
    #[test]
    fn test_value_deserialize_from_simd_json_object() {
        let json_str = json!({"key": "value", "number": 42, "array": [1, 2, 3]}).to_string();
        let mut bytes = json_str.as_bytes().to_vec();
        let parsed: Value = simd_json::from_slice(&mut bytes).expect("simd-json parse failed");
        assert_eq!(
            parsed,
            Value::Json(json!({"key": "value", "number": 42, "array": [1, 2, 3]}))
        );
