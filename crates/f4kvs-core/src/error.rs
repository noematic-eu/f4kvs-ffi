...
//! ```rust
//! use f4kvs_core::{Result, F4KvsError};
//!
...
// Convert from MemoryPoolError
impl From<crate::memory_pool::MemoryPoolError> for F4KvsError {
    fn from(err: crate::memory_pool::MemoryPoolError) -> Self {
        F4KvsError::internal(format!("Memory pool error: {}", err))
    }
}

// Convert from BatchError
impl From<crate::batch_optimizer::BatchError> for F4KvsError {
    fn from(err: crate::batch_optimizer::BatchError) -> Self {
        F4KvsError::internal(format!("Batch operation error: {}", err))
    }
}

// Convert from CompressionError
impl From<crate::compression::CompressionError> for F4KvsError {
    fn from(err: crate::compression::CompressionError) -> Self {
        F4KvsError::internal(format!("Compression error: {}", err))
    }
}

// Convert from simd_json::Error
impl From<simd_json::Error> for F4KvsError {
