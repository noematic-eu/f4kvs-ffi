//! Storage adapter for converting f4kvs_storage::StorageEngine to f4kvs_core::StorageEngine
//!
...
use f4kvs_core::storage_traits::StorageEngine as CoreStorageEngine;
use f4kvs_core::{F4KvsError, Result, Value};
use std::sync::Arc;

/// Adapter to convert f4kvs_storage::StorageEngine to f4kvs_core::StorageEngine
///
...
/// use f4kvs_storage::{create_storage, StorageConfig, StorageBackend};
...
///
/// // Now adapter can be used as a f4kvs_core::StorageEngine
/// adapter.put("key", &f4kvs_core::Value::String("value".to_string())).await?;
/// # Ok(())
/// # }
/// ```
...
impl CoreStorageEngine for StorageAdapter {
    ...
    async fn stats(&self) -> Result<f4kvs_core::StorageStats> {
        let storage_stats = self
            .inner
            .stats()
            .await
            .map_err(|e| F4KvsError::storage(format!("Storage error: {}", e)))?;
        Ok(f4kvs_core::StorageStats {
